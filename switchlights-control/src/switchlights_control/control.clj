...
(defn build-next-state-fn [queue-events next-phase-length-fn]
...
(defn build-next-switch-events-fn [queue-events next-phase-length-fn]
  (let [next-state-fn (build-next-state-fn queue-events next-phase-length-fn)]
    (fn [switch-events]
